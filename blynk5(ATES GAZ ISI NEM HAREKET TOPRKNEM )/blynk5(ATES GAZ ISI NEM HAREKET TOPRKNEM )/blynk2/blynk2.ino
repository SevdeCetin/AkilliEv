...
/*TOPRAK NEM başı*/
const int prob = A4; // sensörümüzden gelen veri ucuna A4 pini atandı
...
/*hareket sensörü baş*/
        int led_hareket_sensoru = 9; // Led pini 13
        int sensor_hareketin = A3 ; // Sesör pini 4
...
/*gaz sensörü son*/
/*hareket sensörü baş*/
        pinMode(led_hareket_sensoru, OUTPUT);
        pinMode(sensor_hareketin, INPUT);
/*hareket sensörü son*/
...
void loop()
{
 ...
 Serial.println("Welcome to TechPonder Flame Sensor Tutorial");
        ...
        if (sensorValue < 100)
        {
          Blynk.virtualWrite(V0, "Ates algılandı.");
        //digitalWrite(led_alev_sensoru,HIGH);
        digitalWrite(buzzer,HIGH);
        }
        else if(sensorValue >= 100){
          Blynk.virtualWrite(V0, "Ates yok.");
...
/*gaz sensörü son*/
     //ısı-nem sensörü başı
        DHT.read11(dht_apin);
        int nem=DHT.humidity;
        int isi=DHT.temperature;
        Blynk.virtualWrite(V2, nem);
        Blynk.virtualWrite(V3, isi);
        delay(2000);
//ısı-nem sensörü son
  /*hareket sensörü baş*/
        val = digitalRead(sensor_hareketin);
        if (val == HIGH) {
        digitalWrite(led_hareket_sensoru, HIGH);
         
        if (state == LOW) {
          Blynk.virtualWrite(V4, "Hareket var");
        state = HIGH;
        }
        }
        else {
        digitalWrite(led_hareket_sensoru, LOW);
         
        if (state == HIGH){
          Blynk.virtualWrite(V4, "Hareket yok.");
        state = LOW;
        }
        }
/*hareket sensörü son*/
  /*TOPRAK-NEM SENSÖRÜ BASLAolcum_sonucu = analogRead(prob); // prob pininden okunan veriyi ölçüm sonucuna kayıt ediyoruz*/
  olcum_sonucu = analogRead(prob);
 
 Blynk.virtualWrite(V5, olcum_sonucu);
 Serial.print(" Toprak İslakligi = ");
 Serial.print(olcum_sonucu); // Ölçülen değeri seri monitöre yazdırıyoruz
 Serial.print("\n");
 delay(500); // seri monitördeki verileri rahat görebilmek veri için akışı yavaşlatıyoruzNGIC*/
  
  /*TOPRAK-NEM SENSÖRÜ BİTİSİ*/
